...
**If you want the hearts to have a custom color or graphic** in the journal for a lobby, you can put the image for it at `Graphics/Atlases/Journal/CollabUtils2Hearts/CollabName/LobbyName.png`. If you want to customize the heart for a particular map in a lobby, put the image at `Graphics/Atlases/Journal/CollabUtils2LevelHearts/CollabName/LobbyName/BinName.png` If the line you want to customize does not match a lobby (for example for a Prologue), put it at `Graphics/Atlases/Journal/CollabUtils2Hearts/CollabName/0-Lobbies/BinName.png` instead.
...
- Put a **Gym Marker** in the room (this is an invisible entity), and change its settings to designate a name (only using letters, numbers and `_`), order, color, and learned color.
